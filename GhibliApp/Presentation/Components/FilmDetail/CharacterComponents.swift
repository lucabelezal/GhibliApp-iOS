...
struct CharacterCard: View {
    ...
    private var infoRows: [CharacterInfoRow] {
        var rows: [CharacterInfoRow] = []

        if let gender = normalized(person.gender) {
            rows.append(CharacterInfoRow(iconName: "person.fill", label: "Gênero", value: gender))
        }

        if let age = normalized(person.age) {
            rows.append(CharacterInfoRow(iconName: "calendar", label: "Idade", value: age))
        }

        if let eyeColor = normalized(person.eyeColor) {
            rows.append(CharacterInfoRow(iconName: "eye", label: "Olhos", value: eyeColor.capitalized))
        }

        if let hairColor = normalized(person.hairColor) {
            rows.append(CharacterInfoRow(iconName: "scissors", label: "Cabelo", value: hairColor.capitalized))
        }

        if person.films.isEmpty == false {
            rows.append(CharacterInfoRow(iconName: "film", label: "Filmes", value: "\(person.films.count)"))
        }
...
struct CharacterInfoRow: Identifiable {
    let id = UUID()
    let iconName: String
    let label: String
    let value: String
...
struct CharacterTraitPlaceholder: View {
    var body: some View {
