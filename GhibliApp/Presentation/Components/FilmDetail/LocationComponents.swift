    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if tagTexts.isEmpty == false {
                LocationTagGroup(tags: tagTexts)
            }
...
struct LocationCardPlaceholder: View {
    private let infoColumns = Array(
        repeating: GridItem(.flexible(), spacing: 10, alignment: .topLeading), count: 2
    )
...
struct LocationTagGroup: View {
    let tags: [String]

    var body: some View {
        HStack(spacing: 6) {
            ForEach(tags, id: \.self) { tag in
                LocationTagView(text: tag)
            }
        }
        .frame(maxWidth: .infinity, alignment: .leading)
    }
...
struct LocationTagGroupPlaceholder: View {
    var body: some View {
        HStack(spacing: 6) {
            ForEach(0..<3, id: \.self) { _ in
                LoadingPlaceholderView()
...
struct LocationTraitPlaceholder: View {
    var body: some View {
...
struct LocationTrait: Identifiable {
    let id = UUID()
    let icon: String
    let label: String
    let value: String
