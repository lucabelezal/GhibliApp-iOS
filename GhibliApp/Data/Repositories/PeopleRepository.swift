...
struct PeopleRepository: PeopleRepositoryProtocol {
    ...
    private func fetchPeopleDTOs(for film: Film) async throws -> [PersonDTO] {
        ...
        if containsCollectionURL(film.people, component: "people") {
            let collection: [PersonDTO] = try await client.request(with: GhibliEndpoint.peopleCollection)
    ...
    private func isCollectionURL(_ url: URL, component: String) -> Bool {
        ...
        return path == "/\(normalized)" || path == "/\(normalized)/" || url.lastPathComponent.lowercased() == normalized
    }

    private func deduplicate<T, Identifier: Hashable>(_ values: [T], id: KeyPath<T, Identifier>) -> [T] {
