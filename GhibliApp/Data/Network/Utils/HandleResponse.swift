...
internal nonisolated func handleResponse<T: Decodable>(
	...
	guard let statusCode = response?.statusCode else { return .failure(.invalidHTTPResponse) }

	return handleResponse(statusCode: statusCode, data: data)
}

private nonisolated func handleResponse<T: Decodable>(statusCode: Int, data: Data?) -> Result<T> {
	switch statusCode {
...
private nonisolated func handleResponse<Model: Decodable>(data: Data?) -> Result<Model> {
